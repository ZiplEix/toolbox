<script lang="ts">
  ...
  const accordionItems = [
    { id: '1', title: 'What is Svelte?', content: 'Svelte is a radical new approach to building user interfaces.' },
    { id: '2', title: 'Why use Tailwind?', content: 'Tailwind CSS is a utility-first CSS framework for rapidly building custom user interfaces.' },
  ];
  ...
  const radioOptions = [
    { value: 'option1', label: 'Default', description: 'This is the default option' },
    { value: 'option2', label: 'Comfortable', description: 'A more comfortable layout' },
    { value: 'option3', label: 'Compact', description: 'Compact view for power users' },
  ];
  ...
  const commandGroups = [
    {
      heading: 'Suggestions',
      items: [
        { id: 'calendar', label: 'Calendar', action: () => console.log('Calendar') },
        { id: 'emoji', label: 'Search Emoji', action: () => console.log('Emoji') },
      ]
    },
    {
      heading: 'Settings',
      items: [
        { id: 'profile', label: 'Profile', shortcut: '⌘P', action: () => console.log('Profile') },
        { id: 'billing', label: 'Billing', shortcut: '⌘B', action: () => console.log('Billing') },
        { id: 'settings', label: 'Settings', shortcut: '⌘S', action: () => console.log('Settings') },
      ]
    }
  ];
  ...
  const timelineItems = [
    { date: 'Just now', title: 'Project created', description: 'You created the project "Purple UI".' },
    { date: '2 hours ago', title: 'Component added', description: 'Added Timeline component.' },
    { date: 'Yesterday', title: 'Initial commit', description: 'Project initialized.' },
  ];
...
<div class="min-h-screen bg-surface p-8 space-y-12">
  <div class="max-w-4xl mx-auto space-y-12">
    <header class="space-y-2">
      <h1 class="text-4xl font-bold text-primary-900">Purple UI Kit</h1>
      <p class="text-lg text-gray-600">A minimalist, elegant UI kit for Svelte 5.</p>
      <div class="pt-4">
    ...
    <section class="space-y-8">
      ...
      <div class="grid md:grid-cols-2 gap-8">
        <div class="space-y-4">
          ...
          <Combobox label="Select Country" options={comboboxOptions} bind:value={comboboxValue} />
        </div>
      ...
      <div class="space-y-4">
        ...
        <div class="h-64 border rounded-lg overflow-hidden">
          <ResizableSplitter>
            {#snippet first()}
              <div class="h-full bg-primary-50 p-4 flex items-center justify-center text-primary-700">
                Left Pane
              </div>
            {/snippet}
            {#snippet second()}
              <div class="h-full bg-white p-4 flex items-center justify-center text-gray-500">
                Right Pane
              </div>
            {/snippet}
          </ResizableSplitter>
      ...
      <div class="flex flex-wrap gap-4 items-center">
        ...
        <Popover>
          ...
          <div class="space-y-2">
            ...
            <p class="text-sm text-gray-500">This is a popover with some content inside.</p>
    ...
    <section class="space-y-8">
      ...
      <div class="space-y-4">
        ...
        <div class="flex gap-2 mt-4">
          <Button size="sm" variant="outline" disabled={currentStep === 0} onclick={() => currentStep--}>Previous</Button>
          <Button size="sm" disabled={currentStep === steps.length - 1} onclick={() => currentStep++}>Next</Button>
        </div>
      </div>
      ...
      <div class="space-y-4">
        ...
        <Drawer bind:open={showDrawer} title="Settings Drawer">
          ...
          {#snippet footer()}
            <Button class="w-full" onclick={() => showDrawer = false}>Close</Button>
          {/snippet}
        </Drawer>
      </div>
    </section>
      ...
      <div class="grid md:grid-cols-2 gap-8">
        <div class="space-y-4">
          ...
          <div class="flex items-center gap-4">
            ...
            <Avatar initials="JD" alt="John Doe" class="bg-primary-100 text-primary-700" />
    ...
    <section class="space-y-4">
      ...
      <div class="flex flex-wrap gap-4">
        ...
        <Button onclick={() => showToast('success')} class="bg-green-600 hover:bg-green-700">Success Toast</Button>
        <Button onclick={() => showToast('warning')} class="bg-yellow-500 hover:bg-yellow-600">Warning Toast</Button>
        <Button onclick={() => showToast('error')} class="bg-red-600 hover:bg-red-700">Error Toast</Button>
      </div>
    </section>
    ...
    <section class="space-y-4">
      ...
      <div class="grid md:grid-cols-2 gap-6 max-w-2xl">
        <Input label="Text Input" placeholder="hello@example.com" bind:value={inputValue} />
        
        <PasswordInput label="Password" placeholder="Enter password" bind:value={passwordValue} />
        
        <div class="col-span-2">
          <h3 class="text-sm font-medium mb-2">Pin Input (OTP)</h3>
          <PinInput length={4} bind:value={pinValue} oncomplete={(v) => toast.add(`Pin entered: ${v}`, 'success')} />
        </div>

        <DatePicker label="Date Picker" bind:value={dateValue} />
        
        <Select label="Select Option" options={selectOptions} bind:value={selectValue} />
        
        <div class="col-span-2">
          <RadioGroup label="View Density" options={radioOptions} bind:value={radioValue} />
        </div>

        <div class="col-span-2">
          <Textarea label="Textarea" placeholder="Type your message here..." bind:value={textareaValue} />
        </div>
    ...
    <section class="space-y-4">
      ...
      <div class="grid md:grid-cols-2 gap-6">
        <Card>
          ...
          {#snippet footer()}
            <Button variant="outline" size="sm" onclick={() => showModal = true}>Open Modal</Button>
          {/snippet}
        </Card>
      </div>
